...
import json # for parsing API requets into dictionaries
...
def main() -> None:
    ...
    # if fileDetails.json()["status"]["code"] > -1: # if this field is present then an error occured
    ...
    for i in fileDetails.json()["response"]["publishedfiledetails"]: #todo: do we need to use enumerate here?
        print(f"{i["title"]} ({i["publishedfileid"]})")
        if logOutput: f.write(f"{i["title"]} ({i["publishedfileid"]})\n")
    f.close()
